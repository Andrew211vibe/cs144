...
void Reassembler::insert( uint64_t first_index, string data, bool is_last_substring, Writer& output )
{
  ...
  if (output.available_capacity() == 0) return; // 没容量缓存则直接返回

  const uint64_t end_idx = first_index + data.size(); // 计算出data的最后一位对应下标
  const uint64_t first_unacceptable_idx = first_unassembled_idx_ + output.available_capacity(); // 第一个不能接受的下标

  /*
   * data不在[first_unassemled_index, first_unacceptable_idx)
   * 也就是要么已经存在ByteStream中，要么超出available capacity，直接丢弃
   */
  if (end_idx <= first_unassembled_idx_ || first_index >= first_unacceptable_idx) return;
...
void Reassembler::buffer_out(Writer& output)
{
  ...
  for (auto iter = buf.begin(); iter != buf.end(); ) {
    if (iter->first > first_unassembled_idx_) break;
